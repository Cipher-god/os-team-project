...
//lock is a variable used to block and unblock the process for entering the critical section
var flag=[0,0];
 //flag is used to indicate whether the process is ready to enter its critical section
...
function entry_state(id){  //entry state 
    ...
    y.innerHTML+='<button id = '+id+' onclick="critical_state(this.id)">P'+id+'</button>'; 
    //if clicked the button then will move to the critical state withs its own id and it also get backs the button after successfull run
}
...
function critical_state(id){  //critical state
    ...
    if(lock==0){  //lock value is 0 so the process can enter the critical state
        y.innerHTML='<button id = '+id+' onclick="exit_state(this.id)">P'+id+'</button>'; 
        //if clicked the button then will move to the exit state.
        turn=1-id;  //gives the turn value
        flag[id-1]=1;
        lock=1; 
        // lock value is set to 1 so another process cannot enter critical section
